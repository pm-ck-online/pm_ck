...
2. **Luôn viết test trước khi giao nộp code.** Dự án có kỷ luật test rất cao
   — hiện tại **603 test, tất cả phải pass** trước khi coi là xong việc nào
...
| `stock_character_classifier.py` | Phân loại "TÍNH CÁCH" giao dịch của TỪNG mã (đặc điểm vận động giá ổn định theo thời gian — KHÁC "giai đoạn thị trường" là xu hướng ngắn hạn): DỨT_KHOÁT_TĂNG/GIẢM (mượt), LINH_XINH (giật cục), BÙNG_NỔ_NGẮN (nhịp bơm ngắn hạn), TRUNG_TÍNH. Dựa trên `choppiness_score` (nghịch đảo Hệ số Hiệu quả Kaufman) + cảnh báo SQUAT/CHURNING. |
| `character_integration.py` | Tích hợp tính cách vào tín hiệu mua/bán + phân bổ vốn đã có — chiết khấu `stock_score` khi mã "LINH_XINH" + tín hiệu BREAKOUT (dễ tín hiệu giả), giảm tỷ trọng phân bổ khi "BÙNG_NỔ_NGẮN"/cảnh báo CHURNING. `quet_tinh_cach_watchlist()` quét cả watchlist, không dừng khi 1 mã lỗi. |
